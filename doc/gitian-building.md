...
*Setup instructions for a Gitian build of Mooncoin using a Debian VM or physical system.*
...
- [Building Mooncoin](#building-Mooncoin)
...
Clone the git repositories for Mooncoin and Gitian and then checkout the Mooncoin version that you want to build.
...
Follow the instructions in [doc/release-process.md](release-process.md) in the Mooncoin repository
...
To build Mooncoin (for Linux, OS X and Windows) just follow the steps under 'perform
Gitian builds' in [doc/release-process.md](release-process.md) in the Mooncoin repository.
...
For example:
```bash
URL=https://github.com/user/Mooncoin.git
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/Mooncoin-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/Mooncoin-build.assert
    gpg --detach-sign ${VERSION}-osx/${SIGNER}/Mooncoin-build.assert
```
...
[Mooncoin/gitian.sigs](https://github.com/Mooncoin-project/gitian.sigs.ltc/) repository, or if that's not possible create a pull
request. 
